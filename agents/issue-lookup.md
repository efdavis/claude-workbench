---
name: issue-lookup
description: Issue tracker bulk data fetcher. Detects Jira or GitHub from URL shape and fetches accordingly. Use for JQL queries, bulk issue lookups, epic trees, sprint boards, or any issue-tracker operation with a large response. For single-issue lookups with scoped fields, call the provider CLI / MCP directly.
model: sonnet
tools:
  - Read
  - Grep
...
- `gh` uses `owner/repo` format for `--repo`.
- For pagination, use `--paginate` on `gh api` (or `--limit N` on `gh issue list` / `gh pr list`).
- Scope `--json` to the fields you need; unscoped list output is large.
