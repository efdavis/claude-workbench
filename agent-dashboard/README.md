...
It **observes and navigates**, and `r` is the one intentional kill path for a stuck or unwanted row. Every external side effect (opening a cmux tab, attaching a tmux lane, ending a live run) is shelled out to `handler.sh`; the renderer's local write is removing a reaped row's card via `emit-status.sh --remove`. No daemon, no cron, no persistent agents.
...
| `r` | **end + reap** the focused row. **live/ghost:** focus/flash its cmux tab (when recorded), close that surface and/or kill its dispatch tmux lane, then remove the card. **dead/stale:** only remove the card (does **not** close a recorded `cmux_surface` — tombstones often share a stale UUID with other rows). Use this for stuck seats you don't need anymore. |
...
- **quota lines** (directly under the title) - one slot per vendor whose data is present: **claude · codex · grok**. Claude always paints: bold **5h** (the wall that stops work) plus **7d** on the same line (`statusline.sh` → dual-write `quota/claude-5h.txt` / `claude-7d.txt` + `/tmp`). Codex paints a **7d** line when present: dashboard refreshes from ChatGPT `wham/usage` (primary window ≈ weekly; falls back to the newest Codex rollout's `rate_limits.primary`) and dual-writes `quota/codex-7d.txt` — only if `~/.codex/auth.json` exists. Grok paints a **7d** line: the **Weekly SuperGrok Limit** from grok.com Settings → Usage (Grok Build + Imagine + API share one pool). Dashboard polls `POST https://grok.com/grok_api_v2.GrokBuildBilling/GetGrokCreditsConfig` (Connect/protobuf) with the Grok CLI OIDC token from `~/.grok/auth.json`, dual-writing `quota/grok-7d.txt` + `/tmp/grok-rate-limit-7d.txt`. There is **no 5-hour Grok window** on that product. Absent `~/.grok`, the grok slot simply doesn't render. After reboot, durable `quota/*` still paints until a live poll re-mirrors.
- **soloists** - every run (role · issue · state · model · ctx · cost · age · pane · note): the `state` cell is glyph-prefixed (🎬 started · 🎻 implementing · 👀 reviewing · 🙋 waiting · 📬 pr-open · 🚨 escalated · 👏 merged · ✅ done · 💀 stale); the `model` column shows which model drives the run (opus/sonnet/haiku color-coded, plus grok/codex peer seats); `ctx` is its context-window fill as a mini bar (red near auto-compact); `cost` is its spend — priced by walking the run's transcript **and every nested subagent/Workflow transcript** via `cost.py` (throttled ~30s), so a fan-out-heavy lane shows its true cost, not the statusline's subagent-blind `total_cost_usd`. Rows it can't bridge to a Claude session (grok/codex seats, panes with no statusline yet) fall back to the mirror value; the `pane` column is `live`/`ghost`/`stale`/`-`. Escalated first, then `waiting` (paused at a human gate, bold yellow), active, and terminal (merged/done) dimmed and kept until you clear them with `r` (or they auto-reap after `AGENT_DASHBOARD_REAP_TERMINAL_HOURS`, default 2 days). Stale runs flagged (no update in 15m); rows capped at `AGENT_DASHBOARD_MAX_ROWS` with an overflow count. The `▸` cursor marks the focused row (reverse-video bar when color is on); `j`/`k`/arrows move it, and it tracks a run by session id so it holds position across a refresh.
...
| `AGENT_DASHBOARD_REAP_TERMINAL_HOURS` | `48` | auto-reap terminal (merged/done) tombstones older than this so the board self-cleans; `<=0` disables (reap only by hand with `r`) |
| `AGENT_DASHBOARD_ESCALATION_NOTIFY` | unset | `1` -> also post a macOS notification (not just the bell) when a **new** escalation appears |
| `NO_BELL` / `AGENT_DASHBOARD_NO_BELL` | unset | set either to mute the terminal bell rung on a new escalation |
...
| `AGENT_DASHBOARD_GROK_WEEKLY_REFRESH` | `60` | seconds between SuperGrok weekly-quota refreshes from `grok.com/.../GetGrokCreditsConfig` |
